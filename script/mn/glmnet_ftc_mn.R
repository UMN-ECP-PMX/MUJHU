...
figDir <- here("deliv/figure/mn/ftc/glmnet")
if(!dir.exists(figDir)) dir.create(figDir)

options(mrggsave.dev="pdf", 
        mrggsave.dir=figDir, 
        mrg.script="glmnet_ftc_mn.R")
...
# Only keep the CLR transformed genus with top 20 abundance
df <- df %>%
  dplyr::select(!contains("genus")) %>%
  bind_cols(yy[, top_genus])%>%
  # Use median for subjects of missing weight 
  mutate(weight = if_else(
    is.na(weight), median(weight, na.rm = TRUE), weight))
...
prep_data <- function(dv, idv){
  ...
  df2 <- df %>% dplyr::select(all_of(dv), contains(idv), 
                              age, weight, ftc_auc_p_d1)
  
  X <- df2 %>% dplyr::select(contains(idv), 
                             age, weight, ftc_auc_p_d1) %>% as.matrix()
...
# Prep data
data <- prep_data("ftc_auc_t_d1", "genus")
...
# Pull top 6 lasso screened genera/variable
top_6_lasso_coeff <- arrange(fit2$lasso$coef, desc(abs(s0))) %>% 
  filter(Variable != "(Intercept)", s0 != 0) %>%
  slice_head(n = 6) %>% 
  mutate(Variable = fct_reorder(Variable, abs(s0), .desc = TRUE)) %>% 
  arrange(Variable) %>% 
  pull(Variable)
