...
figDir <- here("deliv/figure/mn/ftc/randomforest")
if(!dir.exists(figDir)) dir.create(figDir)

options(mrggsave.dev="pdf", 
        mrggsave.dir=figDir, 
        mrg.script="randomforest_ftc_mn.R")
...
colnames(data_gen$X) <- stringr::str_to_sentence(
  gsub("^genus_", "",
       ifelse(grepl("_unclassified$", colnames(data_gen$X)),
              paste0(sub("_unclassified$", "", colnames(data_gen$X)), "(f)"),
              colnames(data_gen$X)))
)
