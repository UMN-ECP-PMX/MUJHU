...
figDir <- here("deliv/figure/3tc/randomforest")
if(!dir.exists(figDir)) dir.create(figDir)

options(mrggsave.dev="pdf", 
        mrggsave.dir=figDir, 
        mrg.script="randomforest_3tc.R")
...
## SHAP summary
p <- shap_beeswarm(mod_phy, data_phy$X, stem = "phylum")
ggsave(
  filename = file.path(figDir, "shap_beeswarm_phylum.pdf"),
  plot     = p,
  width    = 6,
  height   = 6
)
