...
figDir <- here("deliv/figure/3tc/heatmaps")
...
spec <- yspec::load_spec(here(dataDir, "3tc/3tc_expo.yml"))
...
data <- df2 %>% dplyr::select(contains("3tc"), contains("genus")) %>% 
  dplyr::select(-X_3tctp_t, -X_3tc_p) %>% # Remove raw plasma and cervical concentrations
  dplyr::select(-X3tc_ka, -X3tc_vc, -X3tc_clttvvtp, -X3tc_clvvtp,-X3tc_clttve, -X3tc_clvetp, -X3tc_clttvr,-X3tc_kg) #%>% # Remove EBEs
