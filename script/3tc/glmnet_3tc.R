...
figDir <- here("deliv/figure/3tc/glmnet")
if(!dir.exists(figDir)) dir.create(figDir)

options(mrggsave.dev="pdf", 
        mrggsave.dir=figDir, 
        mrg.script="glmnet_3tc.R")
...
# Prep data
data <- prep_data("X3tc_auc_t_d1", "phylum")
...
p1
mrggsave(p1, stem="ridge_coeff_phylum", width=6, height=6)

# Plot lasso coefficients
p2 <- plot_coeff(.x=fit1$lasso$coef)
p2
mrggsave(p2, stem="lasso_coeff_phylum", width=6, height=6)

# Genus 3tc_auc_t_d1 ------------------------------------------------------

# Prep data
data <- prep_data("X3tc_auc_t_d1", "genus")
